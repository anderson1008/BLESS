...
TPZString TPZSimulation :: writeSimulationStatus()
{
   ...
   double replicaNum = 1; // For baseline
   double clockScaleFactor = 1; // For baseline
   
   // For Power evaluation      
   // Power obtained from Synopsis Design Compiler
   // Unit: uW
   // Link power obtained from DSENT
   double RCStaticUnitPower = 0.143;
   double PNStaticUnitPower = 62.88;
   double PAStaticUnitPower = 1.892;
   double LocalStaticUnitPower = 0;
   double XbarStaticUnitPower = 108.95;
   double LatchStaticUnitPower = 31.074;
   double LinkStaticUnitPower = 122.037;  
   double RCDynamicUnitPower = 8.38;
   double PNDynamicUnitPower = 6052.2;
   double PADynamicUnitPower = 127.236;
   double LocalDynamicUnitPower = 0;
   double XbarDynamicUnitPower = 1947.37;
   double LatchDynamicUnitPower = 5967.643;
   double LinkDynamicUnitPower = 9614.458;
   ...
   double LatchNum = 14 * nodes * replicaNum; // this is an estimation, but showing the upper bound of power consumed by latches.
   double LinkNum = 4 * nodes * replicaNum;
   double clockRate = 1.3 * pow (10, -9); // in second
   ...
   double LatchToggleRate = (m_Network->getEventCount( TPZNetwork::LinkTraversal) * 3 + (m_Network->getFlitsTx() + m_Network->getFlitsRx()) * 2) / (LatchNum * m_Clock); // Each network flit toggle all 3 latches at each router. Local inject and eject flits toggle 2 latches.
